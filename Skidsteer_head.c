...
double r = 18.0/1000; //18 for tmp31
double A = 50.0/1000;
...
void* move(void* unused){
    ...
    if(error_type == 1){ error_allowable = 0.01;}
    if(error_type == 0){ error_allowable = 0.005;}
    if(error_type == 2){ error_allowable = 0.001; Kp_X = 50;}
    ...
    while((error>error_allowable)){
        ...
        E[0] = Kp_X*(e[0] + (Ti*e_int[0]));
        E[1] = Kp_Y*(e[1] + (Ti*e_int[1]));
...
int main(void){
    ...
    int n = 1;
    double y = (C[1]-A)*(cos(Home_theta)+sin(Home_theta))+P[1];
    double pi_2 = PI/2;
    while(info_read.data.battery_mV > 4300){ // Return to base if battery drops below 4.3V
        //STEP 1: ROTATION
        printf("Step 1: n = %d\n",n);
        p_ref[0] = p[0]; 
		p_ref[1] = p[1];
        p_ref[2] = PI/2+Home_theta;
        error_type = 0;
        pthread_create(&travel,NULL,&move,NULL);
        pthread_join(travel,NULL);
        usleep(1000000);
